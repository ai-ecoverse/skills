...
name: eslint
description: >
  Run real ESLint over files in the SLICC VFS via a shell command (eslint.jsh),
  built on the ipk-installed `eslint/universal` Linter rather than the eslint
  binary. Lints JavaScript, TypeScript and JSX, plus SLICC's own `.jsh`/`.bsh`
  scripts (whose top-level `await`/`return` normally break a linter's parser).
  Discovers flat config, applies global ignores, writes fixes back, and reports
  in stylish, compact or JSON.
  Use when the agent needs to lint or fix code it wrote or checked out, enforce
  a project's ESLint rules, or check a `.jsh` before shipping it.
  Trigger on requests like "lint this", "run eslint", "check for lint errors",
  "fix the lint errors", "is this file clean", "does this pass eslint",
  "eslint --fix", "check my jsh script", "why is eslint failing here",
  "lint the files I changed". For formatting rather than linting, prefer the
  built-in `biome` command; use this when the project's rules are ESLint's.
allowed-tools: bash
command: eslint
script: scripts/eslint.jsh
---
...
`-f/--format <stylish\|json\|compact>` (`--json` is an alias for the last),
`--ext .js,.mjs`, `--max-warnings <n>`, `--quiet`. Run `eslint --help` for the
full list.
