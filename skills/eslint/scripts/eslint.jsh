...
// `eslint: <message>`, not `Error: <message>`: this command stands in for a
// real CLI, and an agent that has seen ESLint output should recognize its
// diagnostics verbatim.
function fail(message, code) {
  cli.die(message, { prefix: 'eslint', exitCode: code === undefined ? 2 : code });
}

/* ------------------------------- arg parsing ------------------------------ */

function optionValue(args, i, flag) {
  const v = args[i + 1];
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === '-c' || a === '--config') out.config = optionValue(args, i++, a);
    else if (a.startsWith('--config=')) out.config = a.slice('--config='.length);
    else if (a === '--rule') out.rules = parseRules(optionValue(args, i++, a));
    else if (a.startsWith('--rule=')) out.rules = parseRules(a.slice('--rule='.length));
    else if (a === '-f' || a === '--format') out.format = optionValue(args, i++, a);
    else if (a.startsWith('--format=')) out.format = a.slice('--format='.length);
    // Repo convention: every command answers `--json`. Here it is the machine
    // reporter, so it is exactly `--format json` rather than a second shape.
    else if (a === '--json') out.format = 'json';
    else if (a === '--ext') out.extensions = parseExtensions(optionValue(args, i++, a));
    else if (a.startsWith('--ext=')) out.extensions = parseExtensions(a.slice('--ext='.length));
    else if (a === '--max-warnings') out.maxWarnings = parseCount(optionValue(args, i++, a), a);
    else if (a.startsWith('--max-warnings=')) {
      out.maxWarnings = parseCount(a.slice('--max-warnings='.length), '--max-warnings');
    } else if (a === '--stdin-filename') out.stdinFilename = optionValue(args, i++, a);
...
// Color is applied only in the stylish reporter: `compact` is a grep target and
// `json` is a data stream, so neither may carry escapes. `sliccy:color` is a set
// of identity functions when stdout is not a TTY, so this stays safe to pipe.
function formatStylish(results, totals) {
  const lines = [];
  for (const result of results) {
    if (result.messages.length === 0) continue;
    lines.push(color.bold(color.cyan(result.path)));
    ...
    for (const m of result.messages) {
      ...
      const painted = m.severity === 2 ? color.red(kind.padEnd(7)) : color.yellow(kind.padEnd(7));
      const rule = m.ruleId ?? '';
      lines.push(
        `  ${color.dim(pad(pos, posWidth))}  ${painted}  ${m.message}` +
          (rule ? `  ${color.dim(rule)}` : '')
      );
    }
    lines.push('');
  ...
  if (totals.problems > 0) {
    ...
    lines.push(totals.errors > 0 ? color.red(summary) : color.yellow(summary));
