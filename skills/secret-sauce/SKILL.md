...
`.jsh` scripts support `require('package-name')` — modules are fetched from esm.sh CDN. Only static string arguments are pre-scanned (no `require(variable)`). Use npm packages for common tasks instead of hand-rolling utilities:

| Need | Package |
|------|---------|
| Argument parsing | `minimist` |
...
const commands = {
  ...
  async create() {
    const body = JSON.parse(argv._.join(' '));
