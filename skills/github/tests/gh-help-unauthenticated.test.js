...
test('help flags print usage without calling skill.token', async () => {
  ...
  for (const args of cases) {
    ...
    is(result.error?.exitCode, 0, `expected exit 0 for ${JSON.stringify(args)}`);
    is(result.tokenCalls.length, 0, `skill.token must not run for ${JSON.stringify(args)}`);
    ok((/USAGE|SUBCOMMANDS|gh\.jsh/).test(result.stdout.join('\n')));
    ok(!(/No GitHub token/).test(result.error?.message || ''));
  }
});
...
test('gh version and --version work without a token', async () => {
  for (const args of [['version'], ['--version']]) {
    ...
    is(result.error?.exitCode, 0, `expected exit 0 for ${JSON.stringify(args)}`);
    is(result.tokenCalls.length, 0, `skill.token must not run for ${JSON.stringify(args)}`);
    ok((/gh\.jsh \(SLICC GitHub CLI\)/).test(result.stdout.join('\n')));
  }
});
...
test('a missing token still fails a real gh pr list', async () => {
  ...
  ok(result.tokenCalls.length >= 1);
  is(result.tokenCalls[0], 'github');
  ok((/No GitHub token available/).test(result.error.message));
...
test('does not swallow --help as a flag value', async () => {
  ...
  is(asValue.error?.exitCode, 1);
  ok(asValue.tokenCalls.length >= 1);
  ok((/No GitHub token available/).test(asValue.error.message));
  ok(!(/USAGE/).test(asValue.stdout.join('\n')));

  const withToken = await runGh(['issue', 'create', '--body', '-h', '-R', 'octo/repo']);
  is(withToken.error?.exitCode, 1);
  ok(withToken.tokenCalls.length >= 1);
  ok((/title required|usage: gh issue create/i).test(withToken.error.message));
  ok(!(/USAGE/).test(withToken.stdout.join('\n')));
});
